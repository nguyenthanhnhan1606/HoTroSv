<template>
      ...
      <div class="card d-flex mt-1" style="width: 95%; background-color: azure">
        <div class="card-body">
          ...
          <table class="table table-hover table-forum text-center">
            ...
            <tbody>
              <tr v-for="newstt in newstts" :key="newstt.id">
                <td scope="row" class="text-nowrap" style="width: 150px">
                  <router-link
                    :to="{ name: 'newsDetailCl', params: { id: newstt.id } }"
                  >
                    <img :src="newstt.image" :alt="newstt.title" class="w-100" />
                  </router-link>
                </td>
                <td class="text-start">
                  <router-link
                  ...
                  >
                    ...
                    <div class="my-2 mt-1">
                      <div style="font-size: large">
                        <span> {{ newstt.content }} </span>
                      </div>
                      <span>Ngày cập nhật: {{ newstt.ngaycapnhat }}</span>
                    </div>
                  </router-link>
                </td>
                <td>
  ...
  export default {
    ...
    methods: {
      async fetchDataTT(page) {
        if (page < 1) page = 1;
        try {
          let location = endpoints["NewsTT"];
          if (this.searchQuery !== "") {
      ...
      async fetchPageTT() {
        try {
          const response = await Apis.get(endpoints["PageTT"]);
